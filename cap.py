...
def view_all():
   ...
   for row in rows:
      print(f"{row[0] if row[0]!= None else '':<3} {row[1] if row[1]!= None else '':<15} {row[2] if row[2]!= None else '':<15} {row[3] if row[3]!= None else '':<28}{row[4] if row[4]!= None else '':<11}{row[5] if row[5]!= None else '':<15}{row[6] if row[6]!= None else '':<3}{row[7] if row[7]!= None else '':<3}{row[8] if row[8]!= None else '':<7}{row[9] if row[9]!= None else '':<7}")

def search_users():
    entry =f'%{entry}%'
    columns = ['user_id','first_name','last_name','email','phone','password','active','manager','date_created','hire_date']
    print(f"{'user_id':<3} {'first_name':<15} {'last_name':<15} {'email':<28} {'phone':<11} {'password':<15} {'active':<3} {'manager':<3} {'date_created':<9} {'hire_date':<9}")
    ...
    for row in rows:
      print(f"{row[0] if row[0]!= None else '':<3} {row[1] if row[1]!= None else '':<15} {row[2] if row[2]!= None else '':<15} {row[3] if row[3]!= None else '':<28} {row[4] if row[4]!= None else '':<11} {row[5] if row[5]!= None else '':<15} {row[6] if row[6]!= None else '':<3} {row[7] if row[7]!= None else '':<3} {row[8] if row[8]!= None else '':<9} {row[9] if row[9]!= None else '':<9}")
...
def veiw_user_complevel():
    ...
    print(f"{'Competency':<27} {'First':<20} {'Last':<20} {'Level':<3}")
    for row in rows:
        print(f"{row[0] if row[0]!= None else '':<27} {row[1] if row[1]!= None else '':<20} {row[2] if row[2]!= None else '':<20} {row[3] if row[3]!= None else '':<3}")
...
def single_user_report():
   ...
   columns = ['Competency','First','Last','Level']
   print(f"{'Compentency':<17} {'First':<15} {'Last':<15} {'Level':<4}")
   for row in rows:
      print(f"{row[0] if row[0]!= None else '':<17} {row[1] if row[1]!= None else '':<15} {row[2] if row[2]!= None else '':<15} {row[3] if row[3]!= None else '':<4}")
...
def asse_for_user():
   ...
   print(f"{'First':<15} {'Last':<10} {'Score':<4} {'Assessment':<28}")
   for row in rows:
     print(f"{row[0] if row[0]!= None else '':15} {row[1] if row[1]!= None else '':<15} {row[2] if row[2]!= None else '':<4} {row[3] if row[3]!= None else '':<28}")
...
def add_asse_comp():
    ...
    connection.commit()
    print(f"{assessment} added to {competency}!")
...
def edit_user():
   ...
   if user_input == '1':
      ...
      if user_input.lower() =='f':
         ...
         connection.commit()
         print(f"User {u_num}'s first name is now {first}!")
      elif user_input.lower() == 'l':
         last = input('Last Name: ')
         cursor.execute('UPDATE Users SET last_name = ? WHERE user_id=?',(last,u_num))
         connection.commit()
         print(f"User {u_num}'s last name is now {last}!")
      elif user_input.lower()=='b':
         first = input("First Name: ")
         last = input('Last Name: ')
         cursor.execute('UPDATE Users SET first_name=?, last_name = ? WHERE user_id =?',(first,last,u_num))
         connection.commit()
         print(f"User {u_num}'s name is now {first} {last}")

def edit_comp():
   print('To edit a compentency enter the current name of the competency.')
   competency = input('Competency: ')
...
def edit_asse():
   print('Enter the assessment ID of the assessment you would like to edit.')
   assessment_id = input('Assessment ID: ')
   print("Enter what you would like the assessment's name to be. ")
...
def edit_score():
   ...
   if user_choice.lower() == 'c':
      ...
      connection.commit()
      print(f'Competency Assessment Result updated for User {user_id}!')
...
def delete_score():
   ...
   connection.commit()
   print(f"User #{user_id}'s score is removed. ")